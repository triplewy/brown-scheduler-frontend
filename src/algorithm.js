export function algorithm(courses, concentrations, year, concentration, pathways, takenCourses_param) {
    ...
    if (concentration.requirements) {
        if(concentration.code=="ECON-AB"){
            //rest of code is for CS SCB
             console.log(concentration)
            let econ_reqs = concentration.requirements
            for(var econ_class of concentration.requirements.Economics){
                if(econ_class.includes(' or ')){
                    let econ_class_or = econ_class.split(' or ')
                    let taken = true;
                    for(var econ_class_split_class of econ_class_or){
                        if(!takenCourses.includes(econ_class_or)){
                            taken = false
                        }
                    }
                    if(!taken){
                        results.push(econ_class_or[0])
                        side_results.push(econ_class_or[1])
                        requirements_not_taken.push(econ_class_or+' requirement not taken')
                    }
                }else{
                    if(!takenCourses.includes(econ_class)){
                        requirements_not_taken.push(econ_class+' requirement not taken')
                        results.push(econ_class)
                    }
                }
            }
            if(!takenCourses.includes('ECON 0170')){
                results.push('ECON 0170')
                requirements_not_taken.push('Math requirement not taken')
            }
        }else{
